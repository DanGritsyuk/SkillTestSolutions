...
namespace VendingMachine.BLL.Logic
{
    public sealed class PurchaseLogic : IPurchaseLogic
    {
        ...
        {
            _cartCacheStore = cartCacheStore ?? throw new ArgumentNullException(nameof(cartCacheStore));
            ...
            _changeCalculationService = changeCalculationService ?? throw new ArgumentNullException(nameof(changeCalculationService));
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        public async Task<ClientCart> CreateCartAsync(TimeSpan? ttl = null, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<ClientCart> GetCartAsync(Guid cartId, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<ClientCart> AddCoinAsync(
            Guid cartId,
        ...
        public async Task<ClientCart> AddDrinkAsync(
            Guid cartId,
            int drinkId,
            int quantity = 1,
            TimeSpan? ttl = null,
        ...
        public async Task<PurchaseCheckoutResult> CheckoutAsync(Guid cartId, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var dbDrink in dbDrinksById.Values)
            {
                ...
                await _drinksRepository.EditDrink(dbDrink);
            }
        ...
        private async Task<ClientCart> GetRequiredCartAsync(Guid cartId, CancellationToken cancellationToken)
        {
            if (cartId == Guid.Empty) throw new ArgumentException("Cart id is required.", nameof(cartId));
